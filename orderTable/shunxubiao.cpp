/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 2018年06月04日 星期一 19时52分48秒
 ************************************************************************/
#include<stdio.h>
#include<stdlib.h>
#include<iostream>
...
#include<string.h>
#include<inttypes.h>
#define malloc2(size) my_my_malloc(size,__FILE__,__func__,__LINE__)
using namespace std;
...
Vector *init(){
    static int64_t n=100000000000000;
...
int delete_element(Vector *v,int ind){
    if(ind >= v->length || ind < 0) return 0;
...
int main(){
    ...
    v = init();
    for(int i=0;i<10;i++){
        int x=rand()%100, ind=rand()%(i+3);
        printf("%d:insert(%d ,%):",i+1,x,ind);
